...
export default function StatisticsMonthScreen() {
  ...
  const [viewingSpending, setViewingSpending] = useState<MemoTarget | null>(
    null,
  );
  ...
  const { data: range, isLoading: rangeLoading } = useRangeStatistics(
    year,
    month,
    TREND_MONTHS,
  );
  ...
  const renderBudgetRow = (b: CategoryBudget) => {
    const pct =
      b.targetAmount > 0
        ? Math.min(100, Math.round((b.spentAmount / b.targetAmount) * 100))
        : 0;
    ...
    return (
      <Pressable
        key={b.categoryId}
        onPress={() =>
          setEditingBudgetCategory(categoryById.get(b.categoryId) ?? null)
        }
        className="gap-1.5"
      >
        <View className="flex-row items-center justify-between gap-2">
          ...
          <Text
            className={`text-sm font-medium ${over ? "text-red-500" : "text-slate-900 dark:text-white"}`}
          >
        ...
        <View className="h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
          <View
            ...
            style={{
              width: `${Math.max(pct, b.spentAmount > 0 ? 3 : 0)}%`,
  ...
  const memberMax = useMemo(
    () =>
      Math.max(
        1,
        ...(summary?.byMember ?? []).flatMap((m) => [m.income, m.expense]),
      ),
    [summary],
  );
  ...
  const parentTrendSeries = useMemo(() => {
    ...
    for (const point of range?.months ?? []) {
      for (const cat of point.byParentCategory.filter(
        (c) => c.type === "EXPENSE",
      )) {
        if (!names.has(cat.categoryId)) {
          names.set(cat.categoryId, {
            name: cat.categoryName,
            color: cat.color ?? "#105753",
          });
        }
      }
    }
    ...
    const topIds = [
      ...(latest?.byParentCategory.filter((c) => c.type === "EXPENSE") ?? []),
    ]
  ...
  return (
    <Screen maxWidthClassName={isDesktop ? "max-w-[1040px]" : "max-w-[480px]"}>
      ...
      <Link
        href="/statistics"
        className="self-start text-sm font-medium text-primary dark:text-secondary"
      >
        ‹ 월별 표로
      </Link>
          ...
          <View className={isDesktop ? "flex-1 gap-4" : "gap-4"}>
            ...
            <View className="gap-4 rounded-xl bg-white p-4 dark:bg-slate-900">
              <View className="flex-row items-center justify-between">
                <Text className="text-base font-semibold text-slate-900 dark:text-white">
                  {selectedParent
                    ? `${selectedParent.categoryName} 세부`
                    : "대분류별 지출"}
                </Text>
                {selectedParent ? (
                  <Pressable
            ...
            <View className="gap-3">
              ...
              {summary.byMember.length === 0 ? (
                ...
                summary.byMember.map((item) => (
                  <View
                    key={item.userId}
                    className="gap-3 rounded-xl bg-white p-4 dark:bg-slate-900"
                  >
