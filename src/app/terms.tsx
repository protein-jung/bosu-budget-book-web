  ...
  return (
    <>
      ...
      <View className="gap-1">
        ...
        <Text className="text-sm text-slate-400">시행일: 2026년 9월 9일 (이전 버전: 2026년 9월 4일)</Text>
      </View>

      <View className="gap-5">
        <Section title="1. 서비스 소개 및 운영자">
          BOSU Ledger는 가족·룸메이트 등 여러 명이 함께 쓰는 가계부 앱이에요. 초대 코드로
          구성원을 가계부에 초대하면, 구성원들이 입력한 수입·지출·자산 내역을 한 가계부에서
          같이 보고 관리할 수 있어요. 이 서비스는 별도 사업자 등록 없이 개인이 운영하고 있으며, 문의는
        ...
        <Section title="2. 계정 및 가입">
          ...
          않도록 주의해주세요. 타인의 이메일이나 정보를 도용해 가입하거나, 허위 정보로
          가입하거나, 계정을 무단으로 대여·양도하는 행위는 금지돼요. 이런 경우나 부정한
          목적으로 가입을 신청한 경우, 운영자는 가입 승낙을 거부하거나 가입 이후라도 이용계약을
          해지할 수 있어요.
        </Section>
        <Section title="3. 이용자의 의무">
          서비스를 부정한 목적으로 이용하거나, 시스템에 비정상적으로 접근·부하를 주는 행위,
          다른 이용자의 가계부 데이터에 무단으로 접근하려는 행위는 금지돼요. 이를 위반하면
          사전 통지 후 이용을 제한하거나 계정을 정지할 수 있고, 서비스 운영에 미치는 영향이
          긴급한 경우에는 먼저 조치한 뒤 사후에 알려드릴 수 있어요.
        </Section>
        <Section title="4. 입력하는 정보">
          거래 내역, 자산 정보, 카테고리 등 가계부에 입력하는 모든 내용은 이용자 본인이 직접
          작성해요. 입력한 금액과 날짜가 정확한지는 이용자가 확인할 책임이 있어요.
        </Section>
        <Section title="5. 가계부 데이터의 보관 및 백업">
          운영자는 서버와 데이터베이스를 안정적으로 운영하기 위해 노력하지만, 시스템 장애·해킹·
          천재지변 등 예상하지 못한 사유로 데이터가 손상되거나 유실될 가능성을 완전히 배제할 수는
          없어요. 자산·거래 내역 등 중요한 데이터는 통계·내보내기 기능 등을 통해 이용자가 스스로도
          주기적으로 백업해두시길 권장하며, 운영자에게 고의 또는 중대한 과실이 없는 한 이러한
          사유로 발생한 데이터 유실에 대한 책임은 관련 법령이 허용하는 범위에서 제한될 수 있어요.
        </Section>
        <Section title="6. 시세·실거래가 정보에 대한 면책">
        ...
        <Section title="7. 서비스의 제공 및 변경·중단">
          이 서비스는 연중무휴, 1일 24시간 제공을 원칙으로 해요. 다만 시스템 점검·보수·고장,
          정전·통신 장애·천재지변 등 운영자가 통제할 수 없는 사유나 서비스 이용 폭주가 있는
          경우에는 서비스의 전부 또는 일부를 일시 중단할 수 있어요. 가능하면 미리 공지하고,
          긴급한 경우에는 중단 후에 사유를 알려드릴게요. 더 나은 기능을 위해 서비스 내용이
          바뀔 수 있고, 중요한 변경은 사전에 공지해요.
        </Section>
        <Section title="8. 탈퇴">
          설정 화면에서 언제든 계정을 삭제(탈퇴)할 수 있고, 탈퇴 시 계정 정보는 즉시
          삭제돼요. 다만 다른 구성원과 함께 쓰는 가계부의 거래 내역 등은 가계부 자체의 공유
